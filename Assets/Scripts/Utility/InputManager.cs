...
//adrenaline ability block     
    ...
    /// <summary>
    /// Description:
    /// Reads and stores the adrenaline input
    /// Input: 
    /// CallbackContext callbackContext
    /// Return:
    /// void (no return)
    ...
    public void ReadAdrenalineAbilityInput(InputAction.CallbackContext context)
    {
        adrenalinePressed = !context.canceled;
        StartCoroutine(ResetAdrenalinePressed());
    }

    /// <summary>
    /// Description
    /// Coroutine that resets the adrenaline pressed variable after one frame
    /// Input: 
    /// none
    /// Return: 
    /// IEnumerator
    /// </summary>
    /// <returns>IEnumerator: Allows this to function as a coroutine</returns>
    private IEnumerator ResetAdrenalinePressed()
    {
        yield return new WaitForEndOfFrame();
